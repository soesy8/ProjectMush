...
def create_snowbank():
    ...
    ridge_noise = smooth_noise([rng.uniform(-0.18, 0.18) for _ in xs], 2)
    center_noise = smooth_noise([rng.uniform(-0.18, 0.18) for _ in xs], 2)
    width_noise = smooth_noise([rng.uniform(-0.10, 0.10) for _ in xs], 2)
    ...
    for i, x in enumerate(xs):
        u = i / (x_count - 1)
        end_falloff = min(1.0, (u / 0.075), ((1.0 - u) / 0.075))
        end_falloff = 0.55 + 0.45 * max(0.0, min(1.0, end_falloff))
        length_shape = 0.90 + 0.10 * math.sin(math.pi * u) + 0.055 * math.sin(3.2 * math.pi * u + 0.35)
        ...
        for j, y_base in enumerate(ys):
            ...
            cross = max(0.0, 1.0 - abs(normalized) ** 1.55)
            asymmetric = 1.0 - 0.10 * normalized
            z = 0.08 + local_height * (cross ** 0.58) * asymmetric

            # Keep the original overall silhouette, but exaggerate local side-plane
            # displacement so the broad flanks read as lumpy crystalline snow.
            if j not in (0, y_count - 1):
                side_factor = abs(normalized) ** 1.35
                z += rng.uniform(-0.060, 0.060)
                z += rng.uniform(-0.170, 0.170) * side_factor
                y += rng.uniform(-0.040, 0.040)
                y += rng.uniform(-0.140, 0.140) * side_factor

                # The first inset rows form occasional outward chunks rather than
                # making the entire asset uniformly wider.
                if j in (1, y_count - 2):
                    outward = -1.0 if normalized < 0.0 else 1.0
                    y += outward * rng.uniform(-0.045, 0.145)
                if side_factor > 0.42:
                    if (i + j) % 5 == 0:
                        z += 0.125
                    elif (i * 2 + j) % 7 == 0:
            ...
            if i not in (0, x_count - 1):
                x_jitter = rng.uniform(-0.075, 0.075)
            else:
                x_jitter = 0.0
...
def add_preview_scene(asset):
    ...
    bpy.ops.object.camera_add(location=(11.2, -13.2, 7.6))
    camera = bpy.context.object
    camera.name = "Preview_Camera"
    direction = Vector((0.0, 0.0, 0.65)) - camera.location
