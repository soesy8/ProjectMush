Shader "Project Mush/Test_HLSL_Mush"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                // Geometry normal keeps the broad stylized facet and shadow shapes readable.
                // The normal map only affects the URP Lit material response.
                ...
                half metallic = _Metallic;
                    ...
                    metallic = saturate(
                        ...
                        * _MetallicMapStrength);
                #endif

                half smoothness = _Smoothness;
                #if defined(_SMOOTHNESS_MAP)
                    smoothness = saturate(
                        SAMPLE_TEXTURE2D(_SmoothnessMap, sampler_SmoothnessMap, input.uv).r
                        * _SmoothnessMapStrength);
                #endif

                half occlusion = 1.0h;
                #if defined(_AO_MAP)
                    half aoSample = SAMPLE_TEXTURE2D(_AOMap, sampler_AOMap, input.uv).r;
                    // 0 is neutral, 1 is the authored map, and 2 strengthens its contrast.
                    occlusion = saturate(lerp(1.0h, aoSample, _AOMapStrength));
                #endif
