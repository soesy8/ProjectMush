...
public sealed class MushMapRideBootstrap : MonoBehaviour
{
    ...
    [SerializeField, HideInInspector, FormerlySerializedAs("leftDogPrefab")] private GameObject legacyLeftDogPrefab;
    [SerializeField, HideInInspector, FormerlySerializedAs("rightDogPrefab")] private GameObject legacyRightDogPrefab;
    ...
    [SerializeField, HideInInspector, FormerlySerializedAs("rightDogVisual")] private Transform legacyRightDogVisual;
    ...
    private void BuildRideTeam()
    {
        ...
        {
            Debug.LogError("[Mush] 씬에 저장된 썰매 팀이 없습니다. 편집 모드에서 팀을 생성해 주세요.", this);
            return;
        }
        ...
        Vector3 visibleLeftDogPosition = legacyLeftDogPosition;
        Vector3 visibleRightDogPosition = legacyRightDogPosition;
        ...
        DogRuntime leftDog = BuildDog("Left Husky", legacyLeftDogPrefab, teamObject.transform,
            visibleLeftDogPosition, false, 0f);
        DogRuntime rightDog = BuildDog("Right Malamute", legacyRightDogPrefab, teamObject.transform,
    ...
    private bool TryBindSavedRideTeam(Transform mapRoot)
    {
        ...
        if (savedSeat == null || savedSled == null)
            return false;
        ...
        float phase)
    {
        ...
        if (!hasLegPivots && !Application.isPlaying)
        ...
        float phase)
    {
        ...
        if (dogName.Equals("Left Husky", StringComparison.OrdinalIgnoreCase))
        {
            ...
            legacyLeftDogVisual = model.transform;
        }
        else if (dogName.Equals("Right Malamute", StringComparison.OrdinalIgnoreCase))
        {
            legacyRightDogHolder = holder;
            legacyRightDogVisual = model.transform;
        }
        BuildDogLegPivots(dog);
        return dog;
    }
    ...
    private void UpdateMissionTimerText()
    {
        ...
        {
            missionTimerText.text = $"-{Mathf.CeilToInt(sharpCurveOffCourseTimePenalty)}초";
    ...
    private void EnsureResultPanel()
    {
        if (resultPanel != null)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                resultFilledStars[i] = FindDeepChild(resultPanel.transform, $"Flying Result Star {i + 1}");
                resultStarBursts[i] = FindDeepChild(resultPanel.transform, $"Star Impact VFX {i + 1}")?.GetComponent<ParticleSystem>();
                resultFilledStars[i]?.gameObject.SetActive(false);
            }
            foreach (MushQuestRayAction savedButton in resultPanel.GetComponentsInChildren<MushQuestRayAction>(true))
            {
    ...
    private Vector3 StarFlightStart(int index)
    {
        return index switch
        {
            0 => new Vector3(-1.75f, 0.15f, -0.13f),
            1 => new Vector3(0f, 0.15f, -0.13f),
            _ => new Vector3(1.75f, 0.15f, -0.13f),
        };
    }
    ...
    private void EnsureDogTeamVisible()
    {
        ...
        for (int dogIndex = 0; dogIndex < dogs.Count; dogIndex++)
        {
            ...
            if (dog?.visual == null || !dog.holder.gameObject.activeInHierarchy)
                continue;
    ...
    private void ApplyRideDogCustomization()
    {
        ...
        {
            if (dog.member == null || dog.member.CustomizationIndex < 0 || dog.visual == null)
                continue;
            MushCustomizationVisuals.ApplyDogLoadout(dog.visual, dog.member.UseMalamuteAccessories,
                customization, dog.member.CustomizationIndex);
        }
    }

    public void MigrateSceneDogs()
    {
        if (Application.isPlaying)
            return;
        Transform root = rideTeam != null ? rideTeam : FindDeepChild(FindMapRoot(), MushCurvedMapRuntime.RideTeamRootName);
        if (root == null)
            return;
        dogTeamRoot = root;
        Transform[] holders = { legacyLeftDogHolder, legacyRightDogHolder };
        Transform[] visuals = { legacyLeftDogVisual, legacyRightDogVisual };
        string[] oldNames = { "Left Husky", "Right Malamute" };
        for (int i = 0; i < holders.Length; i++)
        {
            Transform holder = holders[i] != null ? holders[i] : FindDeepChild(root, oldNames[i]);
            if (holder == null || holder.GetComponent<MushRideDog>() != null)
                continue;
            Transform visual = visuals[i] != null ? visuals[i] : FindSavedDogVisual(holder, oldNames[i]);
            MushRideDog member = holder.gameObject.AddComponent<MushRideDog>();
            member.Configure(visual, i == 0 ? leftHarness : rightHarness, i, i == 1, i * Mathf.PI);
            holder.name = $"Dog {i + 1:00}";
            if (visual != null) visual.name = "Visual";
            foreach (DogRuntime dog in dogs)
                if (dog.holder == holder) dog.member = member;
        }
    }
    ...
    private void UpdateOffCourseSpeedLimit()
    {
        ...
        if (offCourse && missionTimerStarted)
        {
    ...
    private void SnapRideVisualsToRecoverySurface(Vector3 surfaceNormal)
    {
        ...
        if (sledHolder != null)
            // The sled keeps its authored rotation under the shared slope pivot.
    ...
    private bool UpdateCourseCompletion(bool running)
    {
        ...
        Vector3 endPosition;
        Vector3 endForward;
        if (curvedWorld != null && curvedWorld.TryGetRoutePose(1f, out endPosition, out _, out endForward))
        {
            // The baked route end is authoritative even if an imported finish marker is stale.
        }
        else if (finishMarker != null)
        {
            endPosition = finishMarker.position;
            endForward = finishMarker.forward;
        }
        else return false;
        endForward = Vector3.ProjectOnPlane(endForward, Vector3.up).normalized;
        Vector3 right = Vector3.Cross(Vector3.up, endForward);
        float width = curvedWorld != null ? curvedWorld.RoadHalfWidthMeters + 0.75f : finishDistanceTolerance;
        float previousAlong = Vector3.Dot(previousRidePosition - endPosition, endForward);
        float currentAlong = Vector3.Dot(rideTeam.position - endPosition, endForward);
        float lateral = Mathf.Abs(Vector3.Dot(rideTeam.position - endPosition, right));
        bool crossed = previousAlong <= 0f && currentAlong >= 0f && lateral <= width;
        bool reached = RouteProgress >= 0.995f && Vector3.Distance(rideTeam.position, endPosition) <= width;
        if (!crossed && !reached) return false;
    ...
    private void UpdateSledSurfacePose(bool running)
    {
        ...
        Quaternion targetLocalRotation = seatRestLocalRotation;
