    ...
    public static class MushUiPanelSkin
    {
            ...
            bool preservePrefabLayout = false)
        {
            ...
            if (preservePrefabLayout)
            {
                ...
                rect.localRotation = Quaternion.identity;
                rect.localScale = Vector3.one * Mathf.Min(size.x / SourceWidth, size.y / SourceHeight);
                rect.SetAsFirstSibling();
                ...
                return FindDeepChild(panel.transform, "Image_Inside")?.GetComponent<Image>();
            }
            ...
            // The supplied panel has nested masks with fixed sample dimensions.
            // Stretch the entire clipping chain along with the background.
            foreach (RectTransform child in panel.GetComponentsInChildren<RectTransform>(true))
            {
                if (child.name != "Image_Mask")
                    continue;
