...
namespace Mush.Customization
{
    ...
    public static class MushCustomizationIds
    {
        ...
        public const string FurnitureDogPlay = "furniture_dog_play";
    ...
    public static class MushHousingLayout
    {
        ...
        public const int DogPlayPlacement = 3;
        ...
        public static int PlacementForItem(string itemId) => itemId switch
        {
            ...
            MushCustomizationIds.FurnitureDogPlay => DogPlayPlacement,
            _ => -1,
        };
        ...
        public static Vector3 Position(int placementIndex)
        {
            return placementIndex switch
            {
                ...
                DogPlayPlacement => new Vector3(3.10f, 0f, 0.90f),
                _ => Vector3.zero,
            };
        }
    ...
    public static class MushCustomizationDatabase
    {
        public static readonly MushCustomizationItemDefinition[] Items =
        {
            ...
            new(MushCustomizationIds.SledLantern, "앞 등불", MushItemCategory.Sled, MushEquipmentSlot.SledDecoration, 75),
            new(MushCustomizationIds.DogFedora, "중절모", MushItemCategory.Dog, MushEquipmentSlot.DogHat, 75),
            new(MushCustomizationIds.DogSantaHat, "산타 모자", MushItemCategory.Dog, MushEquipmentSlot.DogHat, 100),
            new(MushCustomizationIds.DogPurpleScarf, "보라 스카프", MushItemCategory.Dog, MushEquipmentSlot.DogNeck, 60),
            new(MushCustomizationIds.DogRedBandana, "빨간 반다나", MushItemCategory.Dog, MushEquipmentSlot.DogNeck, 60),
            new(MushCustomizationIds.FurnitureTable, "작은 탁자", MushItemCategory.Housing, MushEquipmentSlot.Housing, 100),
            new(MushCustomizationIds.FurnitureChair, "포근한 의자", MushItemCategory.Housing, MushEquipmentSlot.Housing, 125),
            new(MushCustomizationIds.FurnitureDogBed, "개 침대", MushItemCategory.Housing, MushEquipmentSlot.Housing, 150),
            new(MushCustomizationIds.FurnitureDogPlay, "공놀이 세트", MushItemCategory.Housing, MushEquipmentSlot.Housing, 80),
        };
    ...
    public sealed class MushCustomizationState
    {
        ...
        [Serializable]
        public sealed class DogEquipment
        {
            public int index;
            public string hat = string.Empty;
            public string neck = string.Empty;
        }
        public List<DogEquipment> additionalDogs = new();

        private DogEquipment FindDogEquipment(int index, bool create)
        {
            additionalDogs ??= new List<DogEquipment>();
            foreach (DogEquipment entry in additionalDogs)
                if (entry != null && entry.index == index) return entry;
            if (!create || index < 2) return null;
            DogEquipment added = new() { index = index };
            additionalDogs.Add(added);
            return added;
        }
        ...
        public string housingDogPlayItem = string.Empty;
        ...
        public string GetDogHat(int dogIndex) => dogIndex == 0 ? dogOneHat : dogIndex == 1 ? dogTwoHat : FindDogEquipment(dogIndex, false)?.hat ?? string.Empty;
        public string GetDogNeck(int dogIndex) => dogIndex == 0 ? dogOneNeck : dogIndex == 1 ? dogTwoNeck : FindDogEquipment(dogIndex, false)?.neck ?? string.Empty;

        public void SetDogHat(int dogIndex, string itemId)
        {
            if (dogIndex == 0) dogOneHat = itemId ?? string.Empty;
            else if (dogIndex == 1) dogTwoHat = itemId ?? string.Empty;
            else if (dogIndex >= 2) FindDogEquipment(dogIndex, true).hat = itemId ?? string.Empty;
        }

        public void SetDogNeck(int dogIndex, string itemId)
        {
            if (dogIndex == 0) dogOneNeck = itemId ?? string.Empty;
            else if (dogIndex == 1) dogTwoNeck = itemId ?? string.Empty;
            else if (dogIndex >= 2) FindDogEquipment(dogIndex, true).neck = itemId ?? string.Empty;
        }
        ...
        public string GetHousingPlacement(int placementIndex)
        {
            return placementIndex switch
            {
                ...
                3 => housingDogPlayItem ?? string.Empty,
                _ => string.Empty,
            };
        }
        ...
        public void SetHousingPlacement(int placementIndex, string itemId)
        {
            ...
            else if (placementIndex == 3) housingDogPlayItem = itemId;
            housingSaveVersion = 2;
        ...
        public void Normalize()
        {
            ...
            housingDogPlayItem ??= string.Empty;
            ...
            if (housingDogPlayItem != MushCustomizationIds.FurnitureDogPlay || !Owns(housingDogPlayItem))
                housingDogPlayItem = string.Empty;
            SyncLegacyHousingFlags();
            ...
            if (additionalDogs != null)
                foreach (DogEquipment entry in additionalDogs)
                {
                    if (entry == null) continue;
                    if (!string.IsNullOrEmpty(entry.hat) && !Owns(entry.hat)) entry.hat = string.Empty;
                    if (!string.IsNullOrEmpty(entry.neck) && !Owns(entry.neck)) entry.neck = string.Empty;
                }
        }
