...
namespace Mush.Customization
{
    ...
    public sealed class MushStoreController : MonoBehaviour
    {
        ...
        private void BuildInterface()
        {
            ...
            CreatePanel(canvas.transform, "LobbyUI_Panel - Store", new Vector2(0f, -90f), new Vector2(1760f, 880f), PanelColor);
            ...
            statusText = CreateText(canvas.transform, "Status", new Vector2(0f, -416f), new Vector2(1320f, 38f),
                22, new Color(1f, 0.82f, 0.48f), string.Empty);
            previewCaption = CreateText(canvas.transform, "Preview Caption", new Vector2(-450f, -96f), new Vector2(570f, 66f),
                26, Color.white, string.Empty);

            GameObject previewObject = new("Product Preview", typeof(RectTransform), typeof(RawImage));
            previewObject.transform.SetParent(canvas.transform, false);
            previewImage = previewObject.GetComponent<RawImage>();
            previewImage.raycastTarget = false;
            previewImage.rectTransform.anchorMin = previewImage.rectTransform.anchorMax = Vector2.one * 0.5f;
            previewImage.rectTransform.anchoredPosition = new Vector2(-450f, -264f);
            previewImage.rectTransform.sizeDelta = new Vector2(560f, 280f);
        ...
        {
            CreateButton(dynamicUi, "썰매", new Vector2(-165f, 48f), new Vector2(280f, 64f),
                () => { customCategory = MushItemCategory.Sled; RefreshPage(); },
                customCategory == MushItemCategory.Sled ? SelectedColor : ButtonColor);
            CreateButton(dynamicUi, "개", new Vector2(165f, 48f), new Vector2(280f, 64f),
        ...
        {
            CreateButton(dynamicUi, "썰매 본체", new Vector2(-165f, -24f), new Vector2(280f, 60f),
                () => { sledPage = SledPage.Body; RefreshPage(); },
                sledPage == SledPage.Body ? SelectedColor : ButtonColor);
            CreateButton(dynamicUi, "장식", new Vector2(165f, -24f), new Vector2(280f, 60f),
        ...
        {
            CreateButton(dynamicUi, "첫째 개", new Vector2(-450f, -24f), new Vector2(260f, 60f),
                () => { selectedDog = 0; RefreshPage(); }, selectedDog == 0 ? SelectedColor : ButtonColor);
            CreateButton(dynamicUi, "둘째 개", new Vector2(-165f, -24f), new Vector2(260f, 60f),
                () => { selectedDog = 1; RefreshPage(); }, selectedDog == 1 ? SelectedColor : ButtonColor);
            CreateButton(dynamicUi, "모자", new Vector2(195f, -24f), new Vector2(240f, 60f),
                () => { dogPage = DogPage.Hat; RefreshPage(); }, dogPage == DogPage.Hat ? SelectedColor : ButtonColor);
            CreateButton(dynamicUi, "목 장비", new Vector2(465f, -24f), new Vector2(240f, 60f),
        ...
        private void BuildOwnedEquipmentGrid(MushEquipmentSlot slot)
        {
            ...
            {
                CreateButton(dynamicUi, "장착 해제", new Vector2(0f, -112f), new Vector2(260f, 96f),
            ...
            for (int index = 0; index < owned.Count; index++)
            {
                ...
                CreateButton(dynamicUi, item.displayName + (equipped ? "\n장착 중" : "\n장착"),
                    position, new Vector2(260f, 96f), () => EquipItem(slot, item.id),
        ...
        {
            CreateButton(dynamicUi, label, new Vector2(x, 48f), new Vector2(270f, 64f),
        ...
        private void RenderProductPreview()
        {
            if (!MushCustomizationVisuals.TryCalculateWorldBounds(previewRoot.gameObject, out Bounds bounds))
            {
            ...
            {
                previewTexture = new RenderTexture(768, 384, 24, RenderTextureFormat.ARGB32);
                previewTexture.Create();
                previewImage.texture = previewTexture;
            }
            GameObject cameraObject = new("Product Snapshot Camera");
            Camera snapshot = cameraObject.AddComponent<Camera>();
            snapshot.enabled = false;
            snapshot.GetUniversalAdditionalCameraData().allowXRRendering = false;
            snapshot.clearFlags = CameraClearFlags.SolidColor;
            snapshot.backgroundColor = Color.clear;
            snapshot.orthographic = true;
            snapshot.aspect = 2f;
            snapshot.orthographicSize = Mathf.Max(0.1f, bounds.size.y * 0.6f, bounds.size.x * 0.3f);
            snapshot.transform.position = bounds.center + Vector3.back * (bounds.extents.z + 10f);
            snapshot.nearClipPlane = 0.1f;
            snapshot.farClipPlane = bounds.size.z + 20f;
            snapshot.cullingMask = 1 << 31;
            snapshot.targetTexture = previewTexture;
            Transform[] parts = previewRoot.GetComponentsInChildren<Transform>(true);
            int[] layers = new int[parts.Length];
            for (int i = 0; i < parts.Length; i++)
            {
                layers[i] = parts[i].gameObject.layer;
                parts[i].gameObject.layer = 31;
            }
            // Render only on a selection change; the same image is used by desktop and VR UI.
        ...
        {
            Image themedPanel = MushUiPanelSkin.CreateCanvasPanel(parent, objectName, position, size, preservePrefabLayout: true);
            if (themedPanel != null)
                return themedPanel;
            ...
            Color color)
        {
            ...
            Text text = CreateText(buttonObject.transform, "Label", Vector2.zero, size - new Vector2(28f, 16f),
                label.Contains("\n") ? 26 : 30, Color.white, label);
            ...
            string content)
        {
            ...
            text.resizeTextMinSize = Mathf.RoundToInt(fontSize * 0.8f);
