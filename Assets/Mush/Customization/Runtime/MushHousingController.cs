...
namespace Mush.Customization
{
    ...
    public sealed class MushHousingController : MonoBehaviour
    {
        ...
        private void BuildLobbyPreview()
        {
            ...
            if (!XRSettings.isDeviceActive) camera.rect = new Rect(0f, 0f, 1f, 0.36f); // VR 전환과 UI 월드 캔버스 설정에서도 같은 카메라를 쓰도록 필드에 저장한다.
        ...
        private void BuildInterface()
        {
            ...
            CreatePanel(canvas.transform, "LobbyUI_Panel - Housing", new Vector2(0f, 130f), new Vector2(1120f, 560f), HeaderColor);
            GameObject backdrop = new("Housing UI Backdrop", typeof(RectTransform), typeof(Image));
            RectTransform backdropRect = backdrop.GetComponent<RectTransform>();
            backdropRect.SetParent(canvas.transform, false);
            backdropRect.anchorMin = new Vector2(0f, 0.36f);
            backdropRect.anchorMax = Vector2.one;
            backdropRect.offsetMin = backdropRect.offsetMax = Vector2.zero;
            backdropRect.SetAsFirstSibling();
            backdrop.GetComponent<Image>().color = previewCamera.backgroundColor;
            backdrop.GetComponent<Image>().raycastTarget = false;
            CreateText(canvas.transform, "Title", new Vector2(-150f, 320f), new Vector2(570f, 50f),
                32, Color.white, "로비 집 꾸미기"); // 하우징 화면 제목을 가운데 표시한다.
            statusText = CreateText(canvas.transform, "Placement Status", new Vector2(0f, -76f),
                new Vector2(950f, 30f), 18, new Color(1f, 0.78f, 0.42f), string.Empty); // 의자/탁자/침대의 실제 장착 상태를 상단에 계속 보여 준다.
            feedbackText = null;
            CreateButton(canvas.transform, "저장하고 로비로", new Vector2(380f, 320f), new Vector2(210f, 48f),
            ...
            dynamicUi.sizeDelta = new Vector2(1008f, 448f); // 브라우저 패널과 같은 크기로 잡아 배치 계산을 단순하게 유지한다.
        }
        ...
        private void RefreshInventory()
        {
            ...
            if (items.Count == 0)
            {
                ...
                CreateText(dynamicUi, "No Models", new Vector2(0f, 100f), new Vector2(900f, 80f),
            ...
            else
            {
                ...
                float cardWidth = 280f; // 작은 네모 카드 하나의 가로 크기다.
                float cardHeight = 190f; // 모델 이미지와 이름이 함께 들어갈 카드 세로 크기다.
                float gap = 32f; // 카드가 여러 개 생겨도 서로 붙지 않게 간격을 둔다.
                ...
                for (int index = 0; index < items.Count; index++)
                {
                    ...
                    CreateModelCard(item, new Vector2(startX + index * (cardWidth + gap), 100f),
                        new Vector2(cardWidth, cardHeight), selected, equipped); // 실제 FBX 스냅샷이 들어간 작은 모델 카드를 만든다.
                }
            }


            CreateButton(dynamicUi, "장착", new Vector2(-165f, -34f), new Vector2(290f, 54f),
                EquipSelectedModel, SelectedColor); // 카드로 선택한 모델을 현재 탭의 고정 슬롯에 실제 장착하는 버튼이다.
            CreateButton(dynamicUi, "빼기", new Vector2(165f, -34f), new Vector2(290f, 54f),
        ...
        private void CreateCategoryTab(int placementIndex, string label, float x)
        {
            ...
            CreateButton(dynamicUi, label, new Vector2(x, 250f), new Vector2(260f, 56f), () =>
            {
            ...
            bool equipped)
        {
            ...
            imageRect.anchoredPosition = new Vector2(0f, 30f); // 모델 이미지와 아래 이름 사이에 여백을 둔다.
            imageRect.sizeDelta = new Vector2(size.x - 28f, size.y - 88f); // 아래쪽 이름 한 줄을 제외한 대부분을 모델 이미지에 쓴다.
            ...
            Text label = CreateText(cardObject.transform, "Card Label", new Vector2(0f, -size.y * 0.5f + 36f),
                new Vector2(size.x - 24f, 60f), 25, Color.white, item.displayName + stateText); // 모델 이름과 장착 여부를 카드 아래에 표시한다.
